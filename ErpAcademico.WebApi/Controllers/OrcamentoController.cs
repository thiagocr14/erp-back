...
public class OrcamentosController : ControllerBase
{
    ...
    {
        var orcamentoId = await _orcamentoService.CriarOrcamento(dto);
        return Ok(new { mensagem = "Orçamento criado com sucesso.", orcamentoId });
    }
    [HttpPost("{id}/converter")]
    public async Task<IActionResult> ConverterEmVenda(Guid id)
    {
        var vendaId = await _orcamentoService.ConverterEmVenda(id);
        return Ok(new { mensagem = "Orçamento convertido em venda com sucesso.", vendaId });
    }
}